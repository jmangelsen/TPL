...
interface Constraint {
  ...
  status: 'constrained' | 'elevated' | 'under-pressure' | 'tightening' | 'limited' | 'stable';
  system: string;
...
const constraints: Constraint[] = [
  ...
  { category: 'Labor', label: 'Labor Tightness', value: '-12%', trend: 'down', status: 'limited', system: 'Human Capital', updatedQuarter: 'Q2 2026' },
  ...
  { category: 'Power', label: 'Substation Capacity', value: 'Limited', trend: 'stable', status: 'limited', system: 'Utility', updatedQuarter: 'Q2 2026' },
...
const StatusBadge = ({ status }: { status: Constraint['status'] }) => {
  ...
  const styles = {
    constrained: 'text-rose-500 border-rose-500/30 bg-rose-500/10',
    elevated: 'text-rose-400/80 border-rose-400/20 bg-rose-400/5',
    'under-pressure': 'text-rose-400/80 border-rose-400/20 bg-rose-400/5',
    tightening: 'text-rose-400/80 border-rose-400/20 bg-rose-400/5',
    limited: 'text-rose-400/80 border-rose-400/20 bg-rose-400/5',
  };

  let label = status.replace('-', ' ');
  if (status === 'limited') label = 'Capacity Limited';
  ...
  return (
    <span className={`inline-flex items-center justify-center px-2.5 h-5 border text-[9px] font-bold uppercase tracking-[0.15em] leading-none transition-all ${styles[status as keyof typeof styles]}`}>
      {label}
    </span>
  );
};
...
export const ConstraintMonitor = ({ user, isSubscribed }: { user: any, isSubscribed: boolean }) => {
  ...
  const isGated = !user;
  const visibleCount = isGated ? 4 : filteredConstraints.length;
    ...
    <div className="min-h-screen bg-[#1a2633] text-slate-200 font-sans selection:bg-[#3b82f6] selection:text-white">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <button 
                ...
                className="p-2 border border-white/5 hover:bg-white/5 transition-colors disabled:opacity-20 text-slate-400 hover:text-white"
                title="Export Data"
              >
                <Download size={14} />
              </button>
      ...
      <div className="max-w-[1600px] mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-12 items-start">
          ...
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-white/5 border border-white/5">
              {[
                ...
                <div key={i} className="bg-[#0f1a24]/50 p-8 flex flex-col justify-between min-h-[140px] hover:bg-[#0f1a24]/70 transition-all group">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-bold uppercase tracking-[0.25em] text-[#3b82f6]">{kpi.label}</span>
                    <TrendIcon trend={kpi.trend as any} />
                  </div>
                  <div className="flex flex-col mt-6 gap-3">
                    <div className="flex flex-col">
                      <span className="text-4xl font-bold text-white tracking-tight leading-none">{kpi.value}</span>
                      {kpi.isIndex && (
              ...
              <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6">
                <div className="flex items-start sm:items-center gap-4">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {categories.map(cat => (
                      <button
                        key={cat}
                        onClick={() => isSubscribed && setActiveCategory(cat)}
                        disabled={!isSubscribed}
                ...
                <div className="relative shrink-0">
                  ...
                  <input 
                    ...
                    className="bg-white/5 border border-white/5 pl-12 pr-6 py-2.5 text-[10px] font-bold uppercase tracking-widest w-full sm:w-64 focus:outline-none focus:border-[#3b82f6]/50 transition-colors placeholder:text-slate-700"
                  />
                </div>
              </div>
              <div className="flex items-start sm:items-center gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  {statuses.map(status => (
                    <button
                      key={status}
                      onClick={() => isSubscribed && setActiveStatus(status)}
                      disabled={!isSubscribed}
                      className={`px-3 py-1.5 text-[9px] font-bold uppercase tracking-widest transition-all border ${
                        activeStatus === status 
            ...
            <div className="bg-[#0f1a24]/50 border border-white/5">
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {filteredConstraints.slice(0, visibleCount).map((c, i) => (
                      <tr key={i} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                        <td className="px-8 py-8">
                          <div className="flex flex-col">
                            <span className="text-sm font-bold text-white group-hover:text-[#3b82f6] transition-colors uppercase tracking-tight">{c.label}</span>
                            <span className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1.5">{c.category}</span>
                          </div>
                        </td>
                        <td className="px-8 py-8">
                          <div className="flex flex-col gap-2">
                            <span className="text-sm font-bold text-slate-200">{c.value}</span>
                            <div className="w-fit">
                        ...
                        <td className="px-8 py-8">
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{c.system}</span>
                        </td>
                        <td className="px-8 py-8 text-right">
                          <span className="text-[10px] text-slate-600 font-bold uppercase tracking-widest">{c.updatedQuarter}</span>
                        </td>
                      </tr>
                    ))}
                  </tbody>
              ...
              {isGated && (
                <div className="p-16 text-center bg-gradient-to-t from-[#1a2633] to-transparent border-t border-white/5">
                  <div className="max-w-sm mx-auto space-y-8">
                    <Lock size={24} className="mx-auto text-[#3b82f6]/40" />
                    <div className="space-y-3">
                      <h3 className="text-sm font-bold uppercase tracking-[0.3em] text-white">Restricted Intelligence</h3>
                      <p className="text-xs text-slate-400 leading-relaxed italic font-serif">
                        Full access to TPL's physical bottleneck monitoring requires a verified analyst account.
                      </p>
                    </div>
                    <button 
          ...
          <aside className="space-y-12">
            <div className="bg-[#0f1a24]/50 border border-white/5 p-10 space-y-10">
              <div className="flex items-center justify-between border-b border-white/5 pb-6">
                <h3 className="text-[11px] font-bold uppercase tracking-[0.3em] text-white">Field Intelligence</h3>
                <span className="text-[9px] font-bold text-[#3b82f6] uppercase tracking-widest">Notes</span>
              </div>
              ...
              <div className="space-y-10">
                {[
                  ...
                  <div key={i} className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-bold uppercase tracking-widest text-[#3b82f6]">{note.type} // {note.system}</span>
                      <span className="text-[9px] text-slate-600 font-bold uppercase tracking-widest">{note.time}</span>
                    </div>
                    <p className="text-[12px] leading-relaxed text-slate-400 font-medium border-l border-[#3b82f6]/30 pl-4 italic">
                      {note.content}
                    </p>
                  </div>
                ))}
            ...
            <div className="bg-[#0f1a24]/50 border border-white/5 p-10 space-y-8">
              <h3 className="text-[11px] font-bold uppercase tracking-[0.3em] text-white">External Signals</h3>
              <div className="space-y-4">
                {[
                  ...
                  <a 
                    ...
                    title={link.tooltip}
                    className="flex items-center justify-between text-[10px] font-bold uppercase tracking-widest text-slate-500 hover:text-[#3b82f6] transition-colors py-3 border-b border-white/5 last:border-0"
                  >
                    <span>
                      {link.label} <span className="text-[8px] text-slate-600 ml-2">{link.date}</span>
                    </span>
        ...
        <div className="mt-24 bg-[#0f1a24]/50 border border-white/5 p-12 lg:p-16 max-w-4xl mx-auto text-center">
          <span className="inline-block text-[10px] font-bold uppercase tracking-[0.3em] text-[#3b82f6] mb-6">
            Methodology
          </span>
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-6 tracking-tight uppercase">
            How the monitor is built
          </h2>
          <p className="text-base text-slate-400 leading-relaxed font-serif italic max-w-2xl mx-auto mb-10">
      ...
      <section className="max-w-[1600px] mx-auto px-6 py-24 border-t border-white/5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-6">
          {['Power', 'Water', 'Permitting', 'Supply Chain', 'Labor'].map(cat => (
            <Link 
              key={cat} 
              to={`/forecast/${cat.toLowerCase().replace(' ', '-')}-forecast`}
              className="p-8 bg-[#0f1a24]/50 border border-white/5 hover:border-[#3b82f6]/30 transition-all group text-center"
            >
              <span className="text-[11px] font-bold text-slate-300 uppercase tracking-widest group-hover:text-white transition-colors">
                {cat}
              </span>
            </Link>
          ))}
      ...
      <footer className="border-t border-white/5 py-12 px-6 mt-24">
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row items-center justify-between gap-8 text-[9px] font-bold uppercase tracking-[0.3em] text-slate-600">
